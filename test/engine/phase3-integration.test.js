...
import { maxSets } from '../../js/engine/optimize.js';
...
// The optimizer half of the pipeline: maximize Modular Frames from 360 ore.
// These tests are about what the physical and belt layers do with a plan, not
// about how the plan was reached, so they take the plan from one place.
const maxMf = () => maxSets({
  dataset: ironChain, caps: capsIron(360), enabledRecipeIds: ALL_IRON_RECIPES,
  targets: [{ itemId: 'mf', weight: 1 }],
});

test('optimize -> realize: 15 Modular Frames build is 50 machines at 0 shards', () => {
  const { recipeRates } = maxMf();
