...
| Base plan **genuinely unbounded** | **Suppress the whole panel.** With output already unlimited, "+28% output" is meaningless and the underlying `sets` is not a trustworthy number. **Amended after the final review:** the original row said "`bounded: false`", which was wrong. `bounded` (`js/engine/expansion.js:872`) conflates *unlimited* with *zero*, because `sets === 0` always leaves `atLimitItems` empty. Gating on it therefore suppressed the zero-output case too — making §5's own Packaged Turbofuel example structurally unreachable, and leaving Expansion silent where the Optimizer offers a one-click fix. The test is now `bounded \|\| sets <= EPS` (`maximizeIsReadable`, `js/ui/expansion.js:501`): suppress only when `!bounded && sets > EPS`. Measured separation over 6522 sampled plans: every `bounded: false` plan had `sets` either exactly 0 or ≥ 2.48e5, nothing between. |
